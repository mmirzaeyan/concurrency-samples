(function ($){
	...
	function createNavigation(resultNum, current, pageSize, targetId) {
		...
		if (current + 1 < pageCount){
	        $('#nextPage' + targetId+' , #lastPage' + targetId).parent().addClass('active').removeClass('disabled');
	        $('#nextPage' + targetId).attr("onclick","$('#"+targetId+"').grid('showPage', "+(current+1)+")").css("cursor","pointer");
	    	$('#lastPage' + targetId).attr("onclick","$('#"+targetId+"').grid('showPage', "+(end-1)+")").css("cursor","pointer");
	    }
	    else{
	    	 $('#nextPage' + targetId+' , #lastPage' + targetId).parent().addClass('disabled').removeClass('active');
	    	 $('#nextPage' + targetId+' , #lastPage' + targetId).attr("onclick",null).css("cursor","");
	    }
	    if (current == 0){
	    	$('#previousPage' + targetId+' , #firstPage' + targetId).parent().addClass('disabled').removeClass('active');
	    	$('#previousPage' + targetId+' , #firstPage' + targetId).attr("onclick",null).css("cursor","");
	    }
	    else{
	    	$('#previousPage' + targetId+' , #firstPage' + targetId).parent().addClass('active').removeClass('disabled');
	    	$('#previousPage' + targetId).attr("onclick","$('#"+targetId+"').grid('showPage', "+(current-1)+")").css("cursor","pointer");
	    	$('#firstPage' + targetId).attr("onclick","$('#"+targetId+"').grid('showPage', "+(start-1)+")").css("cursor","pointer");
	    }
	    if (isNaN(pageCount))
	    	pageCount = 1;
	    var _options = $('#' + targetId).data('gridOptions');
	    ...
	    if (resultNum == 0 &&  current == 0 && pageSize == 0) // yani darim navigation ro reset mikonim
	    	pageCount = 1;
	    $('#gotoPage'+targetId).val(current+1);
	    $('#' + targetId).data('gridOptions', _options);
	    $("#pageNumber"+targetId).text(pageCount.toString()+" صفحه");
	    $("#resultNumber"+targetId).text(resultNum+" مورد");
	}
